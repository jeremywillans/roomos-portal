const debug = require('debug')('roomos-portal:webexService');
...
function webexService() {
  function getPersonId(accessToken, personEmail) {
    return new Promise((resolve, reject) => {
      const options = {
        method: 'GET',
        url: `https://webexapis.com/v1/people?email=${personEmail}`,
        headers:
        {
  ...
  function postxAPI(accessToken, command, deviceId, args) {
    return new Promise((resolve, reject) => {
      const jsonArgs = JSON.parse(args);
      const options = {
        method: 'POST',
        url: `https://webexapis.com/v1/xapi/command/${command}`,
        headers:
        {
          authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
      ...
      axios.request(options)
        .then((response) => {
          // if invalid email - return error
          if (response.status === 400) {
            debug(`server error returned: ${response.status}`);
      ...
      axios(options)
        .then((response) => {
          ...
          response.data.items.forEach((element) => {
            if (element.name === 'roomos-portal Webhook') {
              options = {
                method: 'DELETE',
                url: `https://webexapis.com/v1/webhooks/${element.id}`,
                headers:
                {
                  authorization: `Bearer ${accessToken}`,
                  'Content-Type': 'application/json',
